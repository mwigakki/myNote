...
传统的应用部署方式是通过插件或脚本来安装应用。这样做的缺点是应用的运行、配置、管理、所有生存周期将与当前操作系统绑定，这样做并不利于应用的升级更新/回滚等操作。**解决这一问题通常采用虚拟化技术**，即将物理机的资源分为逻辑上的多个相互隔离的资源组合。**而实现虚拟化通常采用虚拟机或容器技术**。虚拟机通过给定的底层资源利用软件构建出一套完整的操作系统，而容器只是隔离了不同应用程序的运行时环境，不同的容器是共享操作系统的。所以容器相对于虚拟机更加轻量快捷，资源利用率更高。
...
**服务器虚拟化技术**是把一台物理服务器虚拟化成多台逻辑服务器，这种逻辑服务器被称为虚拟机（VM）。通过服务器虚拟化，可以有效地提高服务器的利用率，降低能源消耗，降低运营成本，所以虚拟化技术目前得到了广泛的应用。
...
A. 一个机器上的容器down掉了，如何快速重启一个容器接替它的功能。B. 一个容器的访问量快速增大，如果自动地给它分配更多的资源。C. 当机器发生故障，如何快速将容器迁移到其他机器上以保证服务的可用性？

容器编排就是解决这些问题的。容器编排是指自动化容器的部署、管理、扩展和联网。
...
Docker 包括**三个基本概念、核心概念**:

- **镜像（Image）**：Docker 镜像（Image），是一中轻量级、可执行的独立软件包，它包含运行某个软件所需的所有内容。我们把应用程序和配置依赖全部打包好形成一个可交付的运行环境，这个打包好的运行环境就镜像。
