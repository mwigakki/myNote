...
|                  |                             RIP                              |                             OSPF                             |
| ---------------- | :----------------------------------------------------------: | :----------------------------------------------------------: |
| 基本思想         |              适用距离向量算法找到跳数最少的路径              | 使用**Dijkstra**算法找到最短路径，它额外考虑了链路状态，这个链路状态可以是时延，距离，费用等，由网络管理人员自定义 |
...
首先，TCP算法中发送速率是否发送窗口大小决定的，而发送窗口是由拥塞窗口和接收窗口取较小值决定。接收窗口保证发出取的包对方由足够的空间接收，拥塞窗口保证发出去的包网络有足够带宽承载。而拥塞窗口大小一直由拥塞控制算法进行调控。但一个计算机刚接入网络，首先启动**慢开始算法**，即将拥塞窗口设为一个单位，然后每收到一个数据包的确认就翻倍，目的是试探网络的拥塞情况。知道窗口大小超过满开始门限的值，然后就使用**拥塞避免算法**，此时每次只将拥塞窗口增加一个单位。如果此时数据包确认超时了，就认为出现网络拥塞，那么就重新启动慢开始算法重复上面的过程，同时将慢开始门限设为拥塞时窗口的一半。如果收到了三个连续的重复确认，则认为不是严重的拥塞，此时就启动**快重传算法**，立即重传待确认的报文。同时启动**快恢复算法**，即同时设置拥塞窗口值和慢开始门限为此时窗口值的一半，并启动拥塞避免算法。
...
HTTP是无状态无连接的，每次都发送请求都会在新建的会话上完成
...
- 为了解决这个缺陷，https在http的基础上加入了 SSL/TLS协议（Secure Socket Layer / Transport Layer Secure），来保障了数据传输的安全性。

![img](https://pic2.zhimg.com/80/v2-9556fa61ac7262a18bc639db29362669_720w.webp)

当客户端发起HTTPS请求后，**服务器返回自己的数字证书的公钥**，**客户端验证**数字证书的有效性和时效性，成功后，**生成一个随机值**使用公钥加密后发送给服务器，服务器收到后得到随机值，之后的会话数据传递就**利用这个随机值进行对称加密**了。因为随机值在客户端也有保存，所以在客户端也会使用此随机值对数据进行加密传输。

<img src="img/image-20230329233121212.png" alt="image-20230329233121212" style="zoom:33%;" />

### http和https的区别

https相对于http主要优势在于传输层使用了SSL和TLS这样的**传输层安全协议**。SSL指(Security Socket Layer)安全套接字层，TLS指(Transport Layer Security)传输层安全。传输层安全协议的主要作用是提供通信双方的身份鉴别以及通信数据的加密。而建立加密安全通信的简要过程为，首先客户端和服务器双方协商SSL版本以及加密算法，然后c端鉴别s端发来的包含有RSA公钥的数字证书。接着，c端生成一个秘密数作为通信秘钥，并将此秘钥用RSA公钥加密后发送回s端。自此，对称加密会话通道就建立完成，两端使用c端生成的秘密数进行对称加密通信。保障了通信安全。
...
- **控制平面和数据平面分离**：控制平面由集中式或分布式的控制器组成，负责网络路由决策流量管理等功能。数据平面复杂实际的路由转发。这种解耦合的分离管理使网络管理更具有灵活性。

- **网络开放可编程**：控制平面具有强大的编程计算能力，几年来常常在控制器嵌入机器学习强化学习来指导网络管理。通过数据平面也可以使用如P4等语言实现更加灵活自定的数据包转发行为。
...
OVS （Open vSwitch）是一个高质量的虚拟交换机，基于 SDN 的思想，它可以在linux服务器中虚拟出多个虚拟交换机，这些交换机向下可以挂载操作系统的物理网卡，也可以挂载虚拟网卡。向上可以接收OpenFlow控制器的管理。
...
而在实验室中常见的是KVM（Keyboard Video Mouse）切换器控制台，用来管理和监控堕胎计算机服务器。

### network namespace

首先，namespace是linux内核中的资源抽象方式，也就是虚拟化方式，主要的作用是实现资源的隔离。

其中`network namespace` 就是实现网络相关资源的虚拟化方式。docker默认的网络隔离方式就是使用network namespace实现的。

### docker中的网络模型

docker容器中默认会创建一个名叫docker0的网桥，并分配一个私有的子网的网段。新建的容器都会连接在docker0网桥上。此时不同容器间可以相互通信，但是容器不能和外部相互通信。如果容器想和外部通信，需要使用NAT网络地址转换做端口映射，将宿主机的物理网卡的某个端口映射到容器的IP地址。
...
openstack 是一个针对**云平台的操作系统**。企业可以**借助openstack搭建并管理云计算平台**，对外提供服务，属于IaaS层。openstack将一个或多个服务器的计算，存储，网络资源通过虚拟化的方式抽象成一个大的资源池。它不是一个软件，而通过多个组件构建而成，组件之间通过RESTapi相互调用。这些组件包括提供认证服务的keystone，提供镜像服务的Glance，提供网络服务的Zun等等。

**openstack与docker的区别**

- openstack属于IaaS，是**管理IT资源的工具**，利用openstack将资源抽象化，然后以更灵活的方式分配给用户。
- 而docker本质上是一种**虚拟化技术**。openstack则是虚拟化技术的上层，它是管理这些虚拟机的操作系统。它可以使用docker，KVM等等虚拟化技术。
